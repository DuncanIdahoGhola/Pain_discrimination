...
for p in participants:
    ...
    # ###################################################
    part_path = opj(bidsroot, p)
    deriv_path = opj(part_path, "derivatives")
    print(p)
...
wide_dat["participant"] = wide_dat.index
...
wide_dat = wide_dat[~wide_dat["participant"].isin(wide_dat_placebo)]
...
cohen_dz = val.mean(numeric_only=True) / val.std()
# Evaluation task
fig, ax = plt.subplots()
sns.pointplot(
    x="condition",
...
# Descriptives
wide_dat["mean_age"] = wide_dat["age"].mean(numeric_only=True)
...
wide_dat["ttest_dep_placebo_eval_p"] = p
...
wide_dat.loc["sub-014", "inactive_acc_all"] = 0.5
...
anova_dat[["condition", _, "block"]] = anova_dat["variable"].str.split("_", expand=True)
...
plt.savefig("acc_cond_block.png", dpi=800, bbox_inches="tight")
...
# Delete third legend
axd["A"].legend().remove()
axd["A"].set_ylabel("Proportion correct", fontsize=18)
axd["A"].set_xticks([0, 1])
axd["A"].set_xticklabels(["Placebo on", "Placebo off"], fontsize=12)
axd["A"].set_xlabel("", fontsize=18)
axd["A"].tick_params(labelsize=14)
...
plt.ylim([0, 80])
axd["B"].fill_between(
    np.arange(7.5, 11.5, 0.1), y1=80, facecolor="#2e86c1", alpha=0.2, zorder=0
)
axd["B"].fill_between(
    np.arange(19.5, 24, 0.1),
    y1=80,
...
legend_handles = [
    Line2D([0], [0], color=pal[0], label="TENS on"),     # Use your palette colors
    Line2D([0], [0], color=pal[1], label="TENS off"),
]
...
axd["B"].set_ylabel("Pain rating", fontsize=18)
axd["B"].set_xlabel("Trials", fontsize=18)
axd["B"].set_xticks(np.arange(1, 25))
axd["B"].set_xticklabels(np.arange(1, 25), fontsize=12)
plt.setp(axd["B"].collections[0:2], alpha=0.4)  # for the markers
plt.setp(axd["B"].lines, alpha=1)
plt.tight_layout()
...
axd["C"].set_xticklabels(["Placebo on", "Placebo off"], fontsize=12)
axd["C"].set_ylabel("Pain rating", fontsize=18)
axd["C"].set_xlabel("", fontsize=18)
axd["C"].tick_params(labelsize=14)
...
axd["D"].scatter(diff.mean() / sd_diff_acc, 1, s=90)
axd["D"].plot(
    [
        diff.mean() / sd_diff_acc - h / sd_diff_acc,
        diff.mean() / sd_diff_acc + h / sd_diff_acc,
...
axd["D"].axvline(-0.5, color="k", linestyle="--")
axd["D"].axvline(0.5, color="k", linestyle="--")
axd["D"].tick_params(labelsize=14)
axd["D"].set_xticks([-0.5, 0, 0.5], ["-0.5", "0", "0.5"])
axd["D"].set_ylim([0, 2])
axd['D'].set_yticks([])
axd["D"].set_xlabel("Cohen's dz", fontsize=18)



titles = ["Premier", "Deuxième", "Troisième", "Quatrième", "Cinquième"]
for idx, (label, ax) in enumerate(axd.items()):
    ax.set_title(titles[idx], fontstyle="italic")
    ax.set_title(label, x=-0.15, fontsize=20, fontweight="bold")
